...
def updatetitle(title):
    ...
    global filefound
    if filefound != True:
        title = f'MusicPlayer - Add songs to playlist first! ' + title
        root.title(title)
        root.update()
        return
    title = f'MusicPlayer - Playing:   ' + title
    windows.title(title)
    windows.update()
...
btn4 = Button(addlibrary, text="Recommended Songs",font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,bg=leftFrameColor, ) #Shows recommended songs
btn4.place(x=50,y=170)
...
aboutbtn.place(x=10, y=200)
btn5 = Button(addlibrary, text="User Details",font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,bg=leftFrameColor, ) #Shows recommended songs
